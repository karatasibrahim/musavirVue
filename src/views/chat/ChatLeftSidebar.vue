        ...
        >
          ...
          <ul class="chat-users-list chat-list media-list">
            <chat-contact
              ...
              :key="contact.MukellefId"
              :user="contact"
              tag="li"
              :class="{'active': activeChatContactId === contact.MukellefId}"
              is-chat-contact
              @click="$emit('open-chat', contact.MukellefId)"
            />
          </ul>
...
getMükellef(){
  ...
  setup(props) {
    ...
    const resolveChatContact = userId => props.contacts.find(contact => contact.MukellefId === userId)

    // Search Query
    const searchQuery = ref('')

    const searchFilterFunction = contact => contact.Unvan.toLowerCase().includes(searchQuery.value.toLowerCase())
